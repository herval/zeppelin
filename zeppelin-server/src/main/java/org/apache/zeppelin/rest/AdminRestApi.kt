...
@Path("/admin")
class AdminRestApi(private val adminService: AdminService) {
    ...
    @GET
    @ZeppelinApi
    fun getLoggerSetting(@QueryParam("name") name: String?): List<org.apache.log4j.Logger> {
        logger.debug("name: {}", name)
        return if (null == name || name.isEmpty())
            adminService.loggers
        else
    ...
    @POST
    @ZeppelinApi
    fun setLoggerLevel(loggerRequest: LoggerRequest?): List<org.apache.log4j.Logger> {
        if (null == loggerRequest
                || StringUtils.isEmpty(loggerRequest.name)
                || StringUtils.isEmpty(loggerRequest.level)) {
            logger.trace("loggerRequest: {}", loggerRequest)
            throw BadRequestException("Wrong request body")
        }
        logger.debug("loggerRequest: {}", loggerRequest)

        adminService.setLoggerLevel(loggerRequest)

        return Lists.newArrayList(adminService.getLogger(loggerRequest.name))
    }

    companion object {
        private val logger = LoggerFactory.getLogger(AdminRestApi::class.java)
    }
}
